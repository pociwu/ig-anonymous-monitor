...
| 無法證明作者／共同作者 | 所有 IGWatcher 媒體先進入「歸屬待確認」，保留原四類篩選，不混入一般相簿、預覽或一般媒體統計。頁面標示來源、查詢帳號及不代表已確認作者；預設不發送媒體附件、不自動轉正，也不等同於既有跨帳號污染隔離。正式監控可另依 [Telegram 設定](../README.md#telegram-通知) 明確開啟附標示的新媒體附件；本隔離驗證不啟用 Telegram。 |
...
2026-09-11 針對操作人員回報的兩個帳號核對，Reels 端點分別回傳 1 與 2 筆 `media_type=2`、`is_video=true`、`product_type=feed` 的影片貼文。舊解析器只接受 `clips`，因而重複累積分類失敗；HTTP 請求本身成功。測試使用相同欄位結構及人工 ID／URL，不保存原始回應。

正式保存 adapter 現在把欄位有效、明確標記 `feed` 的影片歸入「歸屬待確認 → 貼文」，不是 Reels；`clips` 維持 Reels 分類。ID、時間、HTTPS 媒體主機、影片與輪播欄位的檢查不變；缺少產品類型、未知類型或互相矛盾的資料仍明確失敗，不會當成空集合。相同來源影片在 Posts 及 Reels 端點出現時沿用同一媒體鍵，舊資料不刪除。獨立嚴格探針沒有放寬。

正常的新觀測會解除舊的分類失敗狀態；曾經通知失敗者會收到一次恢復通知，仍不宣稱完整性驗收通過。分類錯誤通知的「連續 N 次未更新」改為「連續 N 輪擷取或解析未完整成功」，避免把部分可接納內容誤說成完全沒有更新。若更新後仍有其他不合法項目，通知仍可能出現，需要另行診斷。
