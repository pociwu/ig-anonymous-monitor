...
chartConfigs.forEach(config=>{const canvas=document.getElementById(config.canvasId);canvas.addEventListener('pointermove',event=>showChartPoint(event,config));canvas.addEventListener('pointerdown',event=>showChartPoint(event,config));canvas.addEventListener('pointerleave',()=>hideChartPoint(config));drawProfileHistory(config)});
window.addEventListener('resize',()=>chartConfigs.forEach(config=>{document.getElementById(config.tooltipId).hidden=true;config.hoveredIndex=null;drawProfileHistory(config)}));
</script>
</main></body></html>"""
