    ...
    def test_dashboard_shows_saved_profile_id_and_state(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertIn(b'class="media-photo"', detail.data)
                self.assertIn(b'id="photo-lightbox"', detail.data)
                self.assertIn(b'data-lightbox-action="previous"', detail.data)
                self.assertIn(b'data-lightbox-action="next"', detail.data)
                self.assertIn(b'id="lightbox-slideshow"', detail.data)
